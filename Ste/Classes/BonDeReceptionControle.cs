...
{
   public class BonDeReceptionControle
    {
        ProduitService ser_prod = new ProduitService();
        List<Produit> toutProduitDansStock = new List<Produit>();
        public bool BC_Valide(Window_BonDeReception win)
        {
            bool test = true;
            if (win.DateBC.DisplayDate.Year != DateTime.Now.Year)
            {
            ...
            {
                MessageBox.Show("Fournisseur non sélectionné !", "Alerte", MessageBoxButton.OK, MessageBoxImage.Warning);
                test = false;
                return test;
            }
            return test;
